        ...
        if (' ' !== lines[i][0] && '\t' !== lines[i][0]) {        // first line of new entry
          ...
          if (vgapos !== -1 || _3dcontrollerpos !== -1) {         // VGA
            ...
            if (/[\da-fA-F]{2}:[\da-fA-F]{2}\.[\da-fA-F]/.test(pciIDCandidate)) {
              currentController.pciID = pciIDCandidate;
            }
    ...
    process.nextTick(() => {
        ...
        if (util.isRaspberry() && util.isRaspbian()) {
          ...
          exec(cmd, async function (error, stdout) {
            if (!error) {
              ...
              result.controllers = result.controllers.map(({pciID, ...controller}) => ({
                ...controller,
                ...(nvidiaData.find(({pciBus}) => pciBus.endsWith(pciID)) || {})
              }))
            }
      ...
      if (_windows) {
        ...
        try {
          ...
          workload.push(nvidiaDevices);

          Promise.all(
            workload
          ).then(data => {
            const nvidiaData = data[6];

            // controller
            let csections = data[0].split(/\n\s*\n/);
            result.controllers = parseLinesWindowsControllers(csections);
            result.controllers = result.controllers.map(({subDeviceId, ...controller}) => ({
              ...controller,
              ...(nvidiaData.find(device => subDeviceId.toLowerCase() === device.subDeviceId.split('x')[1].toLowerCase()) || {})
            }));
...
function nvidiaDevices(callback = () => {}) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      util.nvidiaSmi().then((stdout, error) => {
        if (error || !stdout) {
          callback([]);
          return resolve([]);
        }
...
exports.nvidiaDevices = nvidiaDevices;
